...
const statusConfig = {
  ...
  cancelled: { label: "Cancelled", dot: "bg-steel", text: "text-light-text-muted dark:text-steel" },
  ...
  return (
    <div className={cn("rounded-xl border border-border-default dark:border-graphite bg-light-bg-secondary dark:bg-midnight-blue/60 p-4 backdrop-blur-sm transition-all duration-200 hover:border-emerald-pro-600/30", className)}>
      <div className="flex items-start gap-3 mb-3">
        <TypeIcon className="h-4 w-4 text-light-text-muted dark:text-steel mt-0.5" />
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-light-text dark:text-white">{appointment.title}</p>
          <p className="text-xs text-light-text-muted dark:text-steel">{type.label}</p>
        </div>
      </div>
      ...
      <div className="space-y-2 text-xs text-light-text-secondary dark:text-silver">
        ...
        <div className="flex items-center gap-2">
          <Clock className="h-3.5 w-3.5 text-light-text-muted dark:text-steel" />
          <span>{appointment.time} ({appointment.duration} min)</span>
        </div>
        <div className="flex items-center gap-2">
          <User className="h-3.5 w-3.5 text-light-text-muted dark:text-steel" />
          <Link
        ...
        <div className="flex items-center gap-2">
          <Building2 className="h-3.5 w-3.5 text-light-text-muted dark:text-steel" />
