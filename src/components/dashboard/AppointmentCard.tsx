...
const statusConfig = {
  upcoming: { label: "Upcoming", dot: "bg-electric-cyan", text: "text-electric-cyan" },
  completed: { label: "Completed", dot: "bg-neon-mint", text: "text-neon-mint" },
  cancelled: { label: "Cancelled", dot: "bg-steel", text: "text-steel" },
...
export function AppointmentCard({ appointment, className }: AppointmentCardProps) {
  ...
  return (
    <div className={cn("rounded-xl border border-graphite bg-midnight-blue/60 p-4 backdrop-blur-sm transition-all duration-200 hover:border-electric-cyan/30", className)}>
      <div className="flex items-start gap-3 mb-3">
        <TypeIcon className="h-4 w-4 text-steel mt-0.5" />
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-white">{appointment.title}</p>
          <p className="text-xs text-steel">{type.label}</p>
        </div>
      </div>
      ...
      <div className="space-y-2 text-xs text-silver">
        ...
        <div className="flex items-center gap-2">
          <User className="h-3.5 w-3.5 text-steel" />
          <Link
      ...
      <div className="mt-4 pt-3 border-t border-graphite/50 flex items-center justify-between">
