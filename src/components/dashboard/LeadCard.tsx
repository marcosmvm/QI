    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {lead.title && (
            <p className="text-xs text-slate-900 dark:text-slate-300 truncate">{lead.title}</p>
          )}
        </div>
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-border-default dark:border-graphite/50 flex items-center justify-between">
        ...
        {lead.score !== undefined && (
          <span className="text-xs text-slate-900 dark:text-slate-300">Score: {lead.score}</span>
        )}
      </div>
    </Link>
  );
