    ...
    <div className={cn("grid grid-cols-2 md:grid-cols-4 gap-4", className)}>
      {stats.map((stat, index) => (
        <div key={index} className="bg-light-bg-secondary dark:bg-midnight-blue border border-border-default dark:border-graphite rounded-xl p-6">
          <p className="text-sm text-slate-900 dark:text-slate-200 font-medium uppercase tracking-wide">
            {stat.label}
          </p>
          <div className="mt-2 flex items-baseline gap-1">
            ...
            {stat.suffix && (
              <span className="text-sm text-slate-900 dark:text-slate-200">{stat.suffix}</span>
            )}
          </div>
