          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-700 dark:text-slate-400">{campaignData.description}</p>
          ...
          {[
            ...
            return (
              ...
              >
                ...
                {metric.sublabel && (
                  <p className="text-xs text-slate-700 dark:text-slate-400 mt-1">{metric.sublabel} total</p>
                )}
              </div>
            );
          })}
              ...
              <div className="flex items-end gap-3 h-48">
                {dailyPerformance.map((day) => (
                  <div key={day.date} className="flex-1 flex flex-col items-center gap-1">
                    ...
                    <span className="text-[10px] text-slate-700 dark:text-slate-400 mt-2">{day.date}</span>
                  </div>
                ))}
              </div>
            </div>
                ...
                {sequences.map((seq) => (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        <span className="text-slate-700 dark:text-slate-400">Open Rate</span>
                        <p className="text-slate-900 dark:text-white font-medium">{seq.openRate}%</p>
                      </div>
                      <div>
                        <span className="text-slate-700 dark:text-slate-400">Reply Rate</span>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-emerald-pro-600/10">
                    <th className="text-left text-xs font-semibold text-slate-700 dark:text-slate-400 uppercase tracking-wider px-6 py-4">Step</th>
                    <th className="text-left text-xs font-semibold text-slate-700 dark:text-slate-400 uppercase tracking-wider px-4 py-4">Name</th>
                    ...
                    <tr key={seq.id} className="hover:bg-emerald-pro-600/5 transition-colors">
                      ...
                      <td className="px-4 py-4 text-center">
                        <span className={cn(
                          ...
                          seq.replyRate >= 5 ? "text-emerald-pro-400" : seq.replyRate >= 3 ? "text-energy-orange" : "text-slate-700 dark:text-slate-400"
                        )}>
                          {seq.replyRate}%
                        </span>
                      </td>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-slate-700 dark:text-slate-400">at {activity.company}</span>
                        </div>
                        <span className="text-xs text-slate-700 dark:text-slate-400">{activity.time}</span>
                      </div>
                      <p className="text-sm text-slate-700 dark:text-slate-400">{activity.message}</p>
                    </div>
                    <Link
