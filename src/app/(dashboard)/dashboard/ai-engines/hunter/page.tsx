...
export default function HunterPage() {
  ...
  const getSentimentColor = (sentiment: string) => {
    switch (sentiment) {
      ...
      default:
        return { bg: "bg-steel/10", text: "text-slate-700 dark:text-slate-400", border: "border-steel/30", icon: Minus };
    }
  };
  ...
  const getActionLabel = (action: string) => {
    switch (action) {
      ...
      default:
        return { label: "Pending Review", color: "text-slate-700 dark:text-slate-400" };
    }
  };
    ...
    <motion.div initial="hidden" animate="visible" variants={containerVariants} className="min-h-screen p-8">
      ...
      <motion.div variants={itemVariants}>
        <div className="mb-8">
          ...
          <p className="text-slate-700 dark:text-slate-400 mt-1">Reply-Based Lead Expansion Engine</p>
        </div>
      </motion.div>
      ...
      <div className="space-y-6">
          ...
          <div className="relative flex items-center justify-between">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-slate-700 dark:text-slate-400 mt-1">Converting positive replies into qualified opportunities</p>
              </div>
            </div>

            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-sm text-slate-700 dark:text-slate-400">Last Processed</p>
                <p className="text-slate-900 dark:text-white font-medium">{overallStats.lastProcessed}</p>
              </div>
              <div className="text-right">
                <p className="text-sm text-slate-700 dark:text-slate-400">Today's Replies</p>
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="glass-premium p-6">
            ...
            <p className="text-sm text-slate-700 dark:text-slate-400 mt-1">Total Replies Processed</p>
            <p className="text-xs text-slate-700 dark:text-slate-400 mt-2">This month</p>
          </div>
          ...
          <div className="glass-premium p-6">
            ...
            <p className="text-sm text-slate-700 dark:text-slate-400 mt-1">Positive Reply Rate</p>
            <p className="text-xs text-slate-700 dark:text-slate-400 mt-2">Industry avg: 45%</p>
          </div>
          ...
          <div className="glass-premium p-6">
            ...
            <p className="text-sm text-slate-700 dark:text-slate-400 mt-1">Leads Qualified</p>
            <p className="text-xs text-slate-700 dark:text-slate-400 mt-2">From positive replies</p>
          </div>
          ...
          <div className="glass-premium p-6">
            ...
            <p className="text-sm text-slate-700 dark:text-slate-400 mt-1">Referrals Generated</p>
            <p className="text-xs text-slate-700 dark:text-slate-400 mt-2">New leads from replies</p>
          </div>
        </motion.div>
          ...
          <div className="glass-premium p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-700 dark:text-slate-400 mt-1">{((replyBreakdown.positive / replyBreakdown.total) * 100).toFixed(1)}% of replies</p>
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-slate-700 dark:text-slate-400 mt-1">{((replyBreakdown.neutral / replyBreakdown.total) * 100).toFixed(1)}% of replies</p>
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-slate-700 dark:text-slate-400 mt-1">{((replyBreakdown.negative / replyBreakdown.total) * 100).toFixed(1)}% of replies</p>
              </div>
            </div>

            <div className="mt-6 pt-4 border-t border-border-default dark:border-graphite/30">
              <div className="flex items-center justify-between text-sm">
                <span className="text-slate-700 dark:text-slate-400">Total Replies</span>
          ...
          <div className="lg:col-span-2 glass-premium p-6">
              ...
              {qualificationPipeline.map((stage, index) => (
                <div key={stage.stage} className="flex-1 relative">
                  <div className={cn(
                    ...
                    <p className="text-xs text-slate-700 dark:text-slate-400 mt-1">{stage.stage}</p>
                  </div>
            ...
            <div className="mt-6 grid grid-cols-3 gap-4">
              <div className="p-4 rounded-lg bg-white dark:bg-deep-space/50 border border-border-default dark:border-graphite/30 text-center">
                ...
                <p className="text-xs text-slate-700 dark:text-slate-400 mt-1">Positive → Qualified</p>
              </div>
              <div className="p-4 rounded-lg bg-white dark:bg-deep-space/50 border border-border-default dark:border-graphite/30 text-center">
                <p className="text-2xl font-sora font-bold text-emerald-pro-500">38%</p>
                <p className="text-xs text-slate-700 dark:text-slate-400 mt-1">Qualified → Meeting</p>
              </div>
              <div className="p-4 rounded-lg bg-white dark:bg-deep-space/50 border border-border-default dark:border-graphite/30 text-center">
                <p className="text-2xl font-sora font-bold text-energy-orange">19%</p>
                <p className="text-xs text-slate-700 dark:text-slate-400 mt-1">Reply → Meeting</p>
              </div>
            </div>
          </div>
        </motion.div>
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-slate-700 dark:text-slate-400 mt-0.5">{reply.receivedAt}</p>
                      </div>
                    </div>

                    <div className="pl-13">
                      <p className="text-sm text-slate-900 dark:text-white mb-1">{reply.subject}</p>
                      <p className="text-sm text-slate-700 dark:text-slate-400 italic">"{reply.preview}"</p>
                      <div className="flex items-center gap-4 mt-3 text-xs">
                        <span className="text-slate-700 dark:text-slate-400">Campaign: <span className="text-emerald-pro-600">{reply.campaign}</span></span>
                        <span className="text-slate-700 dark:text-slate-400">Score: <span className={cn(
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      ...
                      <p className="text-sm text-slate-700 dark:text-slate-400">{lead.title}</p>
                    </div>
                    <span className={cn(
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border-default dark:border-graphite/50">
                  <th className="text-left text-sm font-medium text-slate-700 dark:text-slate-400 pb-4">Campaign</th>
                  <th className="text-center text-sm font-medium text-slate-700 dark:text-slate-400 pb-4">Total Replies</th>
                  <th className="text-center text-sm font-medium text-slate-700 dark:text-slate-400 pb-4">Positive</th>
                  <th className="text-center text-sm font-medium text-slate-700 dark:text-slate-400 pb-4">Qualified</th>
                  <th className="text-center text-sm font-medium text-slate-700 dark:text-slate-400 pb-4">Meetings</th>
                  <th className="text-right text-sm font-medium text-slate-700 dark:text-slate-400 pb-4">Conversion</th>
                </tr>
              </thead>
              <tbody>
