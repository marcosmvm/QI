      ...
      >
        ...
        <div className="h-64 flex items-end gap-2">
          {weeklyData.map((day, i) => (
            ...
            >
              ...
              <span className="text-xs text-slate-900 dark:text-slate-300">{day.date}</span>
            </motion.div>
          ))}
        </div>
        <div className="flex items-center justify-center gap-6 mt-4 pt-4 border-t border-border-default dark:border-graphite/30">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-900 dark:text-slate-300">Emails Sent</span>
          </div>
        </div>
      </motion.div>
          ...
          <div className="space-y-3">
            {campaigns.length === 0 ? (
              <div className="text-center py-8">
                <Mail className="h-8 w-8 text-slate-900 dark:text-slate-300 mx-auto mb-3" />
                <p className="text-slate-900 dark:text-slate-300 text-sm mb-3">No campaigns yet</p>
                <Link
              ...
              campaigns.slice(0, 4).map((campaign, index) => (
                ...
                >
                  <Link
                    ...
                    className="flex items-center justify-between p-3 card-base rounded-xl hover:border-emerald-pro-600/30 hover:-translate-y-0.5 hover:shadow-card-hover transition-all duration-200"
                  >
                      ...
                      className={`text-xs px-2 py-1 rounded-full border capitalize ${
                        campaign.status === 'active'
                          ...
                          : campaign.status === 'paused'
                            ...
                            : 'bg-steel/10 text-slate-900 dark:text-slate-300 border-steel/30'
                      }`}
                    >
                      {campaign.status}
                    </span>
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-slate-900 dark:text-slate-300">{leads.length} new leads</p>
            </div>
            <Link
          ...
          <div className="space-y-3">
            {leads.length === 0 ? (
              <div className="text-center py-8">
                <Users className="h-8 w-8 text-slate-900 dark:text-slate-300 mx-auto mb-3" />
                <p className="text-slate-900 dark:text-slate-300 text-sm mb-3">No leads yet</p>
                <Link
                  href="/dashboard/leads"
                  className="inline-flex items-center gap-2 text-sm text-emerald-pro-600 hover:underline"
                >
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-900 dark:text-slate-300">{lead.company}</p>
                      </div>
                    </div>
                    <span
                      className={`text-xs px-2 py-1 rounded-full border capitalize ${
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs text-slate-900 dark:text-slate-300">View your performance reports</p>
          </div>
        </Link>
        <Link
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs text-slate-900 dark:text-slate-300">Chat with our team</p>
          </div>
        </Link>
        <Link
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs text-slate-900 dark:text-slate-300">Detailed performance insights</p>
          </div>
        </Link>
      </motion.div>
    </motion.div>
