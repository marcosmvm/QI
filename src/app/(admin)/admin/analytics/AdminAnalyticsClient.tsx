  ...
  return (
    <motion.div initial="hidden" animate="visible" variants={containerVariants} className="min-h-screen p-8">
      ...
      <motion.div variants={itemVariants} className="flex items-center justify-between mb-8">
        <div>
          <p className="text-sm text-slate-900 dark:text-slate-200 mb-1">Admin / <span className="text-emerald-pro-600">Analytics</span></p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="glass-premium p-4">
          <div className="flex items-center gap-2 text-slate-900 dark:text-slate-200 mb-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-premium p-6">
          ...
          {topCampaigns.length === 0 ? (
            <div className="text-center py-8">
              <BarChart3 className="h-10 w-10 text-slate-900 dark:text-slate-200 mx-auto mb-3" />
              <p className="text-slate-900 dark:text-slate-200 text-sm">No campaign data yet</p>
            </div>
          ) : (
              ...
              {topCampaigns.slice(0, 5).map((campaign, index) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="text-sm font-mono text-slate-900 dark:text-slate-200 w-6">
                      #{index + 1}
                    </span>
                    <div>
                      ...
                      <p className="text-xs text-slate-900 dark:text-slate-200">{campaign.org}</p>
                    </div>
                  </div>
        ...
        <div className="glass-premium p-6">
          ...
          {clientPerformance.length === 0 ? (
            <div className="text-center py-8">
              <Building2 className="h-10 w-10 text-slate-900 dark:text-slate-200 mx-auto mb-3" />
              <p className="text-slate-900 dark:text-slate-200 text-sm">No client data yet</p>
            </div>
          ) : (
            <div className="space-y-3">
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      <p className="text-xs text-slate-900 dark:text-slate-200">open</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm text-emerald-pro-400">
                        {client.replyRate.toFixed(1)}%
                      </p>
                      <p className="text-xs text-slate-900 dark:text-slate-200">reply</p>
                    </div>
                  </div>
                </div>
              ))}
...
function StatCard({
  ...
  return (
    <div className="glass-premium p-4">
      ...
      <p className="text-xs text-slate-900 dark:text-slate-200 mt-1">{label}</p>
      {target && (
        <p className="text-xs text-slate-900 dark:text-slate-200 mt-1">Target: {target}%</p>
      )}
    </div>
  );
}
