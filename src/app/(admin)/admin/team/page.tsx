    ...
    <motion.div initial="hidden" animate="visible" variants={containerVariants} className="min-h-screen p-8">
      ...
      <motion.div variants={itemVariants} className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-900 dark:text-slate-200 mt-1">Manage your team members and roles</p>
        </div>
      ...
      {teamMembers.length === 0 ? (
        <div className="glass-premium p-12 text-center">
          ...
          <p className="text-slate-900 dark:text-slate-200">No team members yet</p>
...
function TeamMemberCard({ member }: { member: TeamMemberWithProfile }) {
  ...
  const roleColors: Record<string, string> = {
    ...
    viewer: "bg-steel/10 text-slate-900 dark:text-slate-200 border-steel/30",
  };
  ...
  return (
    <div className="glass-premium p-6 hover:border-border-default dark:border-graphite/80 transition-colors">
      <div className="flex items-start justify-between mb-4">
        ...
        <button className="p-2 rounded-lg text-slate-900 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white hover:bg-light-bg-secondary dark:bg-midnight-blue/50 transition-colors">
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2 text-sm">
          <Mail className="h-4 w-4 text-slate-900 dark:text-slate-200" />
          <span className="text-slate-900 dark:text-slate-200 truncate">{profile.email}</span>
        </div>
        {member.hired_at && (
          <div className="flex items-center gap-2 text-sm">
            <Calendar className="h-4 w-4 text-slate-900 dark:text-slate-200" />
