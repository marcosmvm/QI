          ...
          <div className="p-4 border-b border-border-default dark:border-graphite/50 space-y-3">
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-9 rounded-lg border border-border-default dark:border-graphite bg-white dark:bg-deep-space pl-9 pr-3 text-sm text-slate-900 dark:text-white placeholder:text-slate-700 dark:text-slate-400 focus:border-emerald-pro-600/50 focus:outline-none"
              />
            </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {filteredConversations.length === 0 ? (
              <div className="p-8 text-center">
                <Inbox className="h-8 w-8 text-slate-700 dark:text-slate-400 mx-auto mb-2" />
                <p className="text-sm text-slate-700 dark:text-slate-400">No conversations</p>
              </div>
            ) : (
              filteredConversations.map((conv) => (
                ...
                >
                  ...
                  <p className="text-xs text-slate-700 dark:text-slate-400 mb-2">{conv.clientName}</p>
          ...
          {selectedConversation ? (
            <>
              ...
              <div className="px-6 py-4 border-b border-border-default dark:border-graphite/50">
                ...
                <div className="flex items-center gap-3 mt-4">
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-slate-700 dark:text-slate-400">Status:</span>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-slate-700 dark:text-slate-400">Priority:</span>
                    <select className="h-7 px-2 rounded border border-border-default dark:border-graphite bg-white dark:bg-deep-space text-xs text-slate-900 dark:text-white">
                      <option value="low">Low</option>
                      <option value="normal">Normal</option>
                      <option value="high">High</option>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-slate-700 dark:text-slate-400">Assign to:</span>
                    <select className="h-7 px-2 rounded border border-border-default dark:border-graphite bg-white dark:bg-deep-space text-xs text-slate-900 dark:text-white">
              ...
              <div className="p-4 border-t border-border-default dark:border-graphite/50">
                <div className="flex gap-3">
                  <textarea
                    ...
                    rows={3}
                    className="flex-1 rounded-lg border border-border-default dark:border-graphite bg-white dark:bg-deep-space p-3 text-sm text-slate-900 dark:text-white placeholder:text-slate-700 dark:text-slate-400 focus:border-emerald-pro-600/50 focus:outline-none resize-none"
                  />
                  <button
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-slate-700 dark:text-slate-400">Select a conversation</p>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="w-72 border-l border-border-default dark:border-graphite/50 p-4 overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-4 border-t border-border-default dark:border-graphite/50">
                <h4 className="text-xs font-medium text-slate-700 dark:text-slate-400 uppercase tracking-wider mb-3">
                  Conversation Stats
                </h4>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-slate-700 dark:text-slate-400">Messages</span>
                    <span className="text-xs text-slate-900 dark:text-white">
                      {selectedConversation.messages.length}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-slate-700 dark:text-slate-400">Created</span>
                    <span className="text-xs text-slate-900 dark:text-white">
                      {new Date(
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-slate-700 dark:text-slate-400">Assigned to</span>
                    <span className="text-xs text-slate-900 dark:text-white">
...
function QuickStat({
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 glass-premium rounded-lg">
      ...
      <span className="text-xs text-slate-700 dark:text-slate-400">{label}</span>
    </div>
  );
}
...
function StatusBadge({
  ...
  const styles: Record<string, string> = {
    ...
    closed: "bg-steel/10 text-slate-700 dark:text-slate-400 border-steel/30",
  };
...
function PriorityBadge({
  ...
  const styles: Record<string, string> = {
    ...
    normal: "bg-steel/10 text-slate-700 dark:text-slate-400 border-steel/30",
