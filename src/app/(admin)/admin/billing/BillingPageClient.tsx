  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="mb-8">
        <p className="text-sm text-slate-700 dark:text-slate-400 mb-1">Admin / <span className="text-emerald-pro-400">Billing</span></p>
        <h1 className="text-2xl font-sora font-bold text-slate-900 dark:text-white">Billing</h1>
        <p className="text-slate-700 dark:text-slate-400 mt-1">Revenue and subscription management</p>
      </motion.div>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            {subscriptions.filter((s) => s.status === "active").length === 0 ? (
              <div className="text-center py-8">
                <CreditCard className="h-8 w-8 text-slate-700 dark:text-slate-400 mx-auto mb-2" />
                <p className="text-slate-700 dark:text-slate-400 text-sm">No active subscriptions</p>
              </div>
            ) : (
              subscriptions
        ...
        <div className="glass-premium p-6 hover:border-emerald-pro-600/30 hover:-translate-y-0.5 hover:shadow-card-hover transition-all duration-300">
          ...
          <div className="space-y-4">
            {invoices.length === 0 ? (
              <div className="text-center py-8">
                <Receipt className="h-8 w-8 text-slate-700 dark:text-slate-400 mx-auto mb-2" />
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {invoice.status === "paid" ? (
                        ...
                        <Receipt className="h-5 w-5 text-slate-700 dark:text-slate-400" />
                      )}
                    </div>
                    <div>
                      <p className="text-sm font-medium text-slate-900 dark:text-white">
  ...
  return (
    ...
    >
      ...
      <div className="mt-4">
        ...
        <p className="text-xs text-slate-700 dark:text-slate-400 mt-0.5">{subtitle}</p>
      </div>
    </div>
  );
}
