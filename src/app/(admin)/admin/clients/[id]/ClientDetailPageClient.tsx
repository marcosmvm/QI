  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants}>
        ...
        <Link
          href="/admin/clients"
          className="inline-flex items-center gap-2 text-slate-600 dark:text-slate-300 hover:text-emerald-pro-600 transition-colors mb-6"
        >
      ...
      <motion.div variants={itemVariants}>
        <div className="flex items-start justify-between mb-8">
              ...
              <div className="flex items-center gap-4 mt-2">
                {client.domain && (
                  <a
                    ...
                    className="flex items-center gap-1 text-sm text-slate-600 dark:text-slate-300 hover:text-emerald-pro-600 transition-colors"
                  >
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="p-2 border border-border-default dark:border-graphite text-slate-600 dark:text-slate-300 rounded-lg hover:bg-light-bg-secondary dark:bg-midnight-blue/30 transition-colors">
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    ...
    churned: "bg-steel/10 text-slate-600 dark:text-slate-300 border-steel/30",
  };
...
function QuickStat({
  ...
  return (
    <div className="glass-premium p-4 hover:border-emerald-pro-600/30 hover:-translate-y-0.5 hover:shadow-card-hover transition-all duration-300">
      <p className="text-xs text-slate-600 dark:text-slate-300 mb-1">{label}</p>
      <p
      ...
      {subtext && <p className="text-xs text-slate-600 dark:text-slate-300 mt-1">{subtext}</p>}
    </div>
  );
}
