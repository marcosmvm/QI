            ...
            <button
              ...
              className={cn(
                ...
                isActive
                  ...
                  : "border-transparent text-slate-900 dark:text-slate-300 hover:text-slate-900 dark:text-slate-300"
              )}
            >
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              >
                ...
                <span
                  className={cn(
                    ...
                    step.completed ? "text-emerald-pro-400" : "text-slate-900 dark:text-slate-300"
                  )}
                >
                  {step.label}
                </span>
        ...
        <div className="bg-light-bg-secondary dark:bg-midnight-blue/30 border border-border-default dark:border-graphite/50 rounded-xl p-6">
          ...
          <div className="grid grid-cols-4 gap-4">
            <div className="text-center">
              ...
              <p className="text-xs text-slate-900 dark:text-slate-300 mt-1">Emails Sent</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-sora font-bold text-slate-900 dark:text-white">
                {totalOpened.toLocaleString()}
              </p>
              <p className="text-xs text-slate-900 dark:text-slate-300 mt-1">Opens</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-sora font-bold text-emerald-pro-600">
                {overallOpenRate.toFixed(1)}%
              </p>
              <p className="text-xs text-slate-900 dark:text-slate-300 mt-1">Open Rate</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-sora font-bold text-emerald-pro-400">
                {overallReplyRate.toFixed(1)}%
              </p>
              <p className="text-xs text-slate-900 dark:text-slate-300 mt-1">Reply Rate</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-light-bg-secondary dark:bg-midnight-blue/30 border border-border-default dark:border-graphite/50 rounded-xl p-6">
          ...
          {members.length === 0 ? (
            <p className="text-sm text-slate-900 dark:text-slate-300">No team members</p>
          ) : (
            <div className="space-y-3">
              {members.map((member) => (
                <div
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-900 dark:text-slate-300 truncate">{member.email}</p>
                  </div>
                  <span className="text-xs text-slate-900 dark:text-slate-300 capitalize">
                    {member.role}
                  </span>
                </div>
              ))}
          ...
          <div className="bg-light-bg-secondary dark:bg-midnight-blue/30 border border-border-default dark:border-graphite/50 rounded-xl p-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-900 dark:text-slate-300">Plan</span>
                <span className="text-sm text-slate-900 dark:text-white capitalize">
                  {subscription.planType}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-slate-900 dark:text-slate-300">Status</span>
                <SubscriptionBadge status={subscription.status} />
              </div>
              {subscription.monthlyFee && (
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-900 dark:text-slate-300">Monthly Fee</span>
              ...
              {subscription.currentPeriodEnd && (
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-900 dark:text-slate-300">Renews</span>
                  <span className="text-sm text-slate-900 dark:text-white">
  ...
  if (campaigns.length === 0) {
    return (
      <div className="bg-light-bg-secondary dark:bg-midnight-blue/30 border border-border-default dark:border-graphite/50 rounded-xl p-12 text-center">
        <Mail className="h-12 w-12 text-slate-900 dark:text-slate-300 mx-auto mb-4" />
        <p className="text-slate-900 dark:text-slate-300">No campaigns yet</p>
      </div>
    );
  }
...
function LeadsTab({ leads }: { leads: ClientDetailTabsProps["leads"] }) {
  ...
  if (leads.length === 0) {
    return (
      <div className="bg-light-bg-secondary dark:bg-midnight-blue/30 border border-border-default dark:border-graphite/50 rounded-xl p-12 text-center">
        <Users className="h-12 w-12 text-slate-900 dark:text-slate-300 mx-auto mb-4" />
        <p className="text-slate-900 dark:text-slate-300">No leads yet</p>
      </div>
    );
  }
          ...
          <button
            ...
            className={cn(
              ...
              stageFilter === stage.value
                ...
                : "bg-white dark:bg-deep-space/30 text-slate-900 dark:text-slate-300 hover:text-slate-900 dark:text-slate-300 border border-transparent"
            )}
          >
            {stage.label}
            {stage.value !== "all" && (
            ...
            {filteredLeads.map((lead) => (
              ...
              >
                <td className="px-6 py-4">
                  ...
                  <p className="text-xs text-slate-900 dark:text-slate-300">{lead.email}</p>
                </td>
                <td className="px-6 py-4">
                  <p className="text-sm text-slate-900 dark:text-slate-300">{lead.company || "-"}</p>
                ...
                <td className="px-6 py-4">
                  <span className="text-sm text-slate-900 dark:text-slate-300">{lead.score}</span>
                </td>
                <td className="px-6 py-4">
                  <span className="text-sm text-slate-900 dark:text-slate-300">
...
function BillingTab({
  ...
  if (!subscription) {
    return (
      <div className="bg-light-bg-secondary dark:bg-midnight-blue/30 border border-border-default dark:border-graphite/50 rounded-xl p-12 text-center">
        <CreditCard className="h-12 w-12 text-slate-900 dark:text-slate-300 mx-auto mb-4" />
        <p className="text-slate-900 dark:text-slate-300">No subscription</p>
      </div>
    );
  }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-light-bg-secondary dark:bg-midnight-blue/30 border border-border-default dark:border-graphite/50 rounded-xl p-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between p-4 bg-white dark:bg-deep-space/30 rounded-lg">
            <div>
              <p className="text-sm text-slate-900 dark:text-slate-300">Plan</p>
          ...
          {subscription.monthlyFee && (
            <div className="flex items-center gap-3 p-4 bg-white dark:bg-deep-space/30 rounded-lg">
              ...
              <div>
                <p className="text-sm text-slate-900 dark:text-slate-300">Monthly Fee</p>
          ...
          {subscription.currentPeriodEnd && (
            <div className="flex items-center gap-3 p-4 bg-white dark:bg-deep-space/30 rounded-lg">
              ...
              <div>
                <p className="text-sm text-slate-900 dark:text-slate-300">Next Billing Date</p>
                <p className="text-lg font-semibold text-slate-900 dark:text-white">
                  {new Date(subscription.currentPeriodEnd).toLocaleDateString()}
                </p>
              </div>
      ...
      <div className="bg-light-bg-secondary dark:bg-midnight-blue/30 border border-border-default dark:border-graphite/50 rounded-xl p-6">
        ...
        <div className="text-center py-8">
          <FileText className="h-8 w-8 text-slate-900 dark:text-slate-300 mx-auto mb-2" />
          <p className="text-sm text-slate-900 dark:text-slate-300">Invoice history coming soon</p>
        </div>
      </div>
    </div>
  );
...
function NotesTab({
  ...
  > = {
    general: { icon: MessageSquare, color: "text-slate-900 dark:text-slate-300" },
  ...
  return (
    <div>
      ...
      {notes.length === 0 ? (
        <div className="bg-light-bg-secondary dark:bg-midnight-blue/30 border border-border-default dark:border-graphite/50 rounded-xl p-12 text-center">
          <FileText className="h-12 w-12 text-slate-900 dark:text-slate-300 mx-auto mb-4" />
          <p className="text-slate-900 dark:text-slate-300">No notes yet</p>
        </div>
      ) : (
        <div className="space-y-4">
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="h-8 w-8 rounded-lg bg-white dark:bg-deep-space/50 flex items-center justify-center">
        <Icon className="h-4 w-4 text-slate-900 dark:text-slate-300" />
      </div>
      <div>
        <p className="text-xs text-slate-900 dark:text-slate-300">{label}</p>
...
function IntegrationItem({
  ...
  return (
    <div className="flex items-center justify-between p-3 bg-white dark:bg-deep-space/30 rounded-lg">
      <div className="flex items-center gap-2">
        <Icon className="h-4 w-4 text-slate-900 dark:text-slate-300" />
        <span className="text-sm text-slate-900 dark:text-slate-300">{label}</span>
      </div>
      {connected ? (
        ...
        <span className="text-xs text-slate-900 dark:text-slate-300">Not connected</span>
      )}
    </div>
  );
}
...
function CampaignStatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    ...
    draft: "bg-steel/10 text-slate-900 dark:text-slate-300 border-steel/30",
...
function SubscriptionBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    ...
    canceled: "bg-steel/10 text-slate-900 dark:text-slate-300 border-steel/30",
    paused: "bg-steel/10 text-slate-900 dark:text-slate-300 border-steel/30",
  };
...
function StageBadge({ stage }: { stage: string }) {
  const styles: Record<string, string> = {
    new: "bg-steel/10 text-slate-900 dark:text-slate-300 border-steel/30",
