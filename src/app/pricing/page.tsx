        ...
        <Container className="relative z-10">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-electric-cyan/10 border border-electric-cyan/20 mb-6">
              ...
              <span className="text-sm font-medium text-electric-cyan">Transparent Pricing</span>
            </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {pricingTiers.map((tier, index) => (
              <motion.div
                ...
                className={cn(
                  "relative p-8 rounded-3xl border bg-gradient-to-br from-midnight-blue/80 to-deep-space/90 transition-all duration-300",
                  tier.popular
                    ? "border-electric-cyan/50 shadow-[0_0_40px_rgba(0,212,255,0.2)] scale-105 z-10"
                    : "border-electric-cyan/20 hover:border-electric-cyan/40"
                )}
              >
                {/* Popular Badge */}
                {tier.popular && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-gradient-to-r from-electric-cyan to-quantum-violet text-deep-space text-sm font-semibold">
                    Most Popular
                  </div>
                )}
                ...
                <div className="text-center mb-8">
                  <div className="flex items-baseline justify-center gap-1">
                    <span className="text-5xl font-sora font-bold text-transparent bg-clip-text bg-gradient-to-r from-electric-cyan to-quantum-violet">
                      {tier.price}
                    </span>
                    <span className="text-steel">{tier.priceSubtext}</span>
                  </div>
                  <p className="text-silver text-sm mt-2">{tier.description}</p>
                </div>
                  ...
                  {tier.features.map((feature, featureIndex) => (
                    ...
                    >
                      {feature.included ? (
                        ...
                        <X className="h-5 w-5 text-steel flex-shrink-0 mt-0.5" />
                      )}
                      <span className={cn(
                        "text-sm",
                        feature.included ? "text-silver" : "text-steel"
                      )}>
                        {feature.text}
                      </span>
                    </li>
                ...
                <Link href="/contact">
                  <Button
                    className={cn(
                      ...
                      tier.popular
                        ? "bg-gradient-to-r from-electric-cyan to-cyan-dark hover:from-cyan-light hover:to-electric-cyan text-deep-space"
                        : "bg-transparent border border-electric-cyan/30 hover:bg-electric-cyan/10 text-white"
                    )}
                  >
                    {tier.cta}
